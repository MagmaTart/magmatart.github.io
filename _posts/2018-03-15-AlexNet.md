---
layout: article
title:  "[논문] ImageNet Classification with Deep Convolutional Neural Networks"
date: 2018-03-15 21:00:00 -0400
modify_date: 2018-03-15 21:00:00 -0400
tags:
- Deep Learning
- Paper Reading
- CNN
...
::memo: __논문 리딩 스터디 #17__: 이미지 인식 / 컴퓨터 비전 분야에 딥 러닝을 성공적으로 도입한 모델인 AlexNet 논문을 읽고 정리해봅니다.

<!--more-->
-----
논문 링크: [ImageNet Classification with Deep Convolutional Neural Networks](https://papers.nips.cc/paper/4824-imagenet-classification-with-deep-convolutional-neural-networks.pdf)
...
#### ReLU Nonlinearity
보통 사용했던 뉴런의 Activation Function은 Sigmoid ( $$f(x) = (1 + e^{-x})^{-1}$$ ), 또는 tanh ( $$f(x) = \tanh(x)$$ )였습니다. 논문에서는 새로운 Nonlinearity Function인 ReLU ( $$f(x) = \max(0, x)$$ )를 제안합니다. 이 함수가 다른 Activation들보다 Gradient Descent를 통한 학습이 빠르다고 합니다. 다음과 같은 그림으로 차이를 보여주고 있습니다. (손으로 그린 티가 확 나긴 하지만...)

<p align="center"><image width="400" src="/assets/posts/images/AlexNet/figure1.PNG"/></p>
...
<p align="center"><image width="200" src="/assets/posts/images/AlexNet/Lateral.png"/></p>
...
<p align="center"><image src="/assets/posts/images/AlexNet/LRN.png"/></p>

[이미지 출처](https://www.quora.com/What-is-local-response-normalization)
...
$$b_{x,y}^i = a_{x,y}^i / \Bigg( k+\alpha \sum^{\min(N-1, i+n/2)}_{j=\max(0, 1-n/2)} (a_{x,y}^j)^2 \Bigg)^\beta$$

$$n$$은 Spatial Convolution을 수행할 인접한(Adjacent) Kernel의 개수이고, $$N$$은 레이어 내 총 커널의 개수입니다. $$k, \ n, \ \alpha, \ \beta$$는 모두 Hyper-Parameter이고, Validation Set을 이용하여 튜닝한 결과 $$k=2, \ n=5, \ \alpha=10^{-4}, \ \beta = 0.75$$을 사용하는게 좋다고 결과를 내렸다고 합니다.
...
<p align="center"><image width="500" src="/assets/posts/images/AlexNet/overlapping.png"/></p>

Pooling을 Stride $$s$$, Kernel size $$(z \times z)$$로 수행할 때, $$s = z$$이면 일반 Pooling입니다. Overlapping Pooling은 $$s < z$$인 경우입니다. 논문에서는 $$s=2, \ z=3$$을 적용하였고, $$s=2, \ z=2$$일 때와 비교했을 때 Top-5와 Top-1 Error Rate가 각각 0.4%, 0.3% 감소했다고 합니다.
...
<p align="center"><image src="/assets/posts/images/AlexNet/Architecture.PNG"/></p>
...
<p align="center"><image src="/assets/posts/images/AlexNet/augmentation.png"/></p>
...
<p align="center"><image src="/assets/posts/images/AlexNet/Augmentation2.PNG"/></p>
...
### Results & Conclusions
<p align="center"><image width="300" src="/assets/posts/images/AlexNet/table1.PNG"/></p>

<p align="center"><image width="400" src="/assets/posts/images/AlexNet/table2.PNG"/></p>
